...
globals [
  q1         ; the first atom
  q2         ; the second atom that the user moves
...
to setup
  ...
  let particle-size 20
  set r-min particle-size * 1.5
...
to update-force  ; particle procedure
  ...
  set force calc-force r

  ; Separate force into x and y components
  face q2
  set fx force * dx * step-size
  set fy force * dy * step-size
end

to-report calc-force [r]

  report (- 12 / (r ^ 7)) * (r-min ^ 12 / r ^ 6 - r-min ^ 6)  ;; Force of the Lennard-Jones potential with epsilon = 1
...
to move  ; particle procedure
  ...
  set heading 0
  ifelse abs (ycor + vy) < max-pycor [
    fd vy
  ] [
    let d-to-wall(max-pycor - abs ycor) * (sign ycor)
    fd d-to-wall
    set vy (- vy)
    fd (vy - d-to-wall)
  ]
  ...
  set heading 90
  ifelse abs (xcor + vx) < max-pxcor [
    fd vx
  ] [
    let d-to-wall (max-pxcor - abs xcor) * (sign xcor)
    fd d-to-wall
    set vx (- vx)
    fd (vx - d-to-wall)
  ]
  ...
  if abs xcor >= max-pxcor [set vx (- vx)]
...
to fade-patches
  ask patches with [pcolor != black] [
    set pcolor max list (pcolor + fade-rate * 0.005) black
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
