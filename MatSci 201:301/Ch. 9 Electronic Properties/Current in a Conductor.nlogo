...
to move
  ...
  (ifelse
    new-x > max-pxcor [set Vx 0] ; if electron is at the right edge, set Vx to 0
    new-x > min-pxcor [
      ; not at the end, move
      setxy new-x (ycor + Vy)
    ] [  ;; if at the end, calculate current
      pen-up
...
;  let new-direction random-float 360
;  set Vx (cos new-direction) * speed
;  set Vy (sin new-direction) * speed
end
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "if ticks > 0 [ plotxy ticks current ]"
...
<experiments>
  <experiment name="experiment" repetitions="3" runMetricsEveryStep="false">
    ...
    <enumeratedValueSet variable="number-of-electrons">
      <value value="400"/>
    </enumeratedValueSet>
  </experiment>
</experiments>
