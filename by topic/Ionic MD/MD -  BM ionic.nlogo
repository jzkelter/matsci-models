...
to setup
  ...
  set dt .1
  set kb (1 / 10000)  ; just picking a random constant for Kb that makes things work reasonably
  set r-min 2.4
...
to setup-atoms
  ...
  (ifelse
    initial-arrangement = "Solid" [
      ...
      let offset -1  * (l - 1) * diameter / 2
      ask atoms [
        let row floor (who / l)
        let col (remainder who l)
        set xcor offset + row * diameter
        set ycor offset + col * diameter
        set charge ifelse-value ((row + col) mod 2 = 0) [1] [-1]
        set color ifelse-value charge > 0 [red] [blue]
      ]
...
to update-force-and-velocity  ; atom procedure
  ...
  ask other atoms  [
    ...
    let force calc-force r charge [charge] of myself
    face myself
...
to-report calc-force-with [other-turtle]
  report calc-force (distance other-turtle) charge [charge] of other-turtle
end

to-report calc-force [r q1 q2]
...
;  let force (-48 * eps / r ) * (twelfth-power - (1 / 2) * sixth-power) + .0001
  if r < .45 [print "small r!"]
  let A1 100  ; Constant in the Born-Mayer potential
  let B 10  ; Contant for strength of Coulombic interaction
  let alpha 2  ; Constnat in the exponent of the Born-Mayer potential

  let force A1 * alpha * exp(-1 * alpha * r) + B * q1 * q2 / (r ^ 2)
  report force
end
