...
globals [
  ...
  n-pinned-atoms  ; how many atoms are pinned on the left and right
]
...
to setup-atoms-and-links-and-force-lines
  ...
  set-pinned-and-externally-forced-under-shear
...
to create-dislocation [x-dist y-dist]
  let curr-y-cor (min [ycor] of atoms) + n-pinned-atoms
  ...
  while [ curr-y-cor <= ceiling (ymax) ] [
    ask atoms with [
      ...
      and ycor >= curr-y-cor ] [
      (ifelse
        xcor <= curr-x-cor + x-dist * .75 and xcor >= curr-x-cor [ die ]
...
to set-pinned-and-externally-forced-under-shear
  ...
  ask atoms with [
        ((xcor <= x-min + 0.5 or xcor >= x-max - 0.5) and ( ycor <= y-min + n-pinned-atoms)) ; bottom n-pinned-atoms on left and on right
  ...
  let forced-atoms atoms with [ ycor >= y-min + n-pinned-atoms and (distancexy force-line-xcor ycor) <= 1]
...
to calculate-fl-positions ; (calculate new force line positions)
  ...
  ifelse (external-force + -1 * equalizing-LJ-force) = 0 [
...
to adjust-force
  if precision prev-length 6 >= precision (right-edge - left-fl) 6 [
    set external-force precision (external-force + .0005) 3
  ]
  ; increments external-force-auto if the sample has reached an equilibrium or if the
...
to-report report-new-force
  ...
  report external-force / num-forced-atoms
end
...
to-report stress ; tension only
  ...
  report (-1 * ((-1 * external-force) + equalizing-LJ-force) / min-A)
end

to-report report-indiv-ex-force
  report (external-force + -1 * equalizing-LJ-force) / num-forced-atoms
end

to-report report-total-ex-force
  report external-force + -1 * equalizing-LJ-force
end
...
935
295
external force per forced atom
...
80
310
Click empty areas to add atoms
11
0.0
1
