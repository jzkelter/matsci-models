...
to create-the-holes
  set pcolor blue
  if random-float 1 < p-doping-frac [
...
to create-the-electrons
  set pcolor green - 2
  if random-float 1 < n-doping-frac [
    set pcharge (- e-charge)
    sprout-electrons 1 [
      electron-init
    ]
...
;*** Move Procedures***
to move
  ; Move using velocity-verlet integration

  let new-x velocity-verlet-pos xcor vx fx
  let new-y ycor + vy * time-step
  facexy new-x new-y
  fd distancexy new-x new-y
end

to update-force-and-velocity
  ;; This is calculated even if scattering happens so that fx can be updated at the end
  let k_e 1
  ...
  let new-fx k_e * charge * ((1 - del-x) *  ([e-field] of left-meter) + (del-x * ([e-field] of right-meter)))

  ifelse (random-float 1 < scatter-prob * time-step) ;or (pxcor = min-pxcor) or (pxcor = max-pxcor)
...
to calc-charges
  ...
  ask boundary-meters [
    set x-charge (external-voltage / 2) * (- sign x)
    set running-ave-charge (weighted-running-ave running-ave-charge x-charge)]
...
934
500
Hole (white) and Electron (black) Distributions
NIL
NIL
0.0
5.0
...
"set-proper-x-range\n\n" "clear-plot\nset-proper-x-range\n\n"
PENS
"holes" 1.0 0 -4539718 true "" "foreach sorted-meters\n[ m -> ask m [plotxy (x - 0) running-ave-h-count]]\n\n"
"electrons" 1.0 0 -16777216 true "" "foreach sorted-meters \n[ m -> ask m [plotxy (x - 0) running-ave-e-count]]\n"
"y-axis" 1.0 0 -4539718 true "" "plot-y-axis"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" ";if ticks > current-delay [plotxy ticks (charge-flow / ticks)]\nif ticks > (current-delay + 20) [plotxy ticks (charge-flow / (ticks - (current-delay )))]"
...
191
358
Current won't start plotting until 70 ticks to wait for equilibration.
11
0.0
1
...
"" "\n "
PENS
"holes distribution" 0.2 1 -5516827 true "" "histogram [vx] of holes"
"y-axis" 1.0 0 -7500403 true "" "foreach (list plot-y-min plot-y-max)\n[y-cor -> plotxy 0 y-cor]"
"hole mean" 1.0 0 -13791810 true "" "plot-pen-reset\nlet mean-vx mean [vx] of holes\nforeach (list plot-y-min plot-y-max)\n[y-cor -> plotxy mean-vx y-cor]"
"electrons distribution" 0.2 1 -1604481 true "" "histogram [vx] of electrons"
"electron mean" 1.0 0 -2674135 true "" "plot-pen-reset\nlet mean-vx mean [vx] of electrons\nforeach (list plot-y-min plot-y-max)\n[y-cor -> plotxy mean-vx y-cor]"
...
"" ""
PENS
"default" 0.2 1 -16777216 true "" "histogram [vx] of electrons"
"y-axis" 1.0 0 -7500403 true "" "foreach (list plot-y-min plot-y-max)\n[y-cor -> plotxy 0 y-cor]"
...
"" ""
PENS
"default" 0.2 1 -16777216 true "" "histogram [vx] of holes"
"y-axis" 1.0 0 -7500403 true "" "foreach (list plot-y-min plot-y-max)"
"pen-2" 1.0 0 -13791810 true "" "plot-pen-reset\nlet mean-vx mean [vx] of holes\nforeach (list plot-y-min plot-y-max)\n[y-cor -> plotxy mean-vx y-cor]"
