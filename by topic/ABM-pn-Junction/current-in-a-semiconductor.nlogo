...
to move
  ...
  if random-float 1 < scatter-prob * time-step [init-velocity]
  let new-x velocity-verlet-pos xcor vx fx
  let new-y ycor + vy * time-step ; There are only forces acting in x direction, so just moves with constant velocity.
  facexy new-x new-y
  let old-pxcor pxcor
