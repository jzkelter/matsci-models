; THIS VERSION KEEPS THE CONTACTS CONSTANT MANUALLY instead of just letting charges continue
; to wrap around and counting current that way.
...
to hold-contacts-charge-constant
  ...
  ;; Hold left contact charge constant
  let left-charge-sum xcor-charge-sum min-pxcor
  update-charge-flow (- left-charge-sum) ; negative charge on left contact is positive current
  (ifelse
    left-charge-sum < 0  [
      init-holes-at-x-cor abs left-charge-sum min-pxcor
    ]

    left-charge-sum > 0  [
      let surplus-h up-to-n-of (left-charge-sum) holes with [pxcor = min-pxcor]

      if count surplus-h < left-charge-sum [
        init-electrons-at-x-cor (left-charge-sum - count surplus-h) min-pxcor
      ]
  ...
  ;; Hold right contact charge constant
  let right-charge-sum xcor-charge-sum max-pxcor
  update-charge-flow right-charge-sum ; positivei charge on right contact is positive current
  (ifelse
    right-charge-sum < 0  [
      let surplus-e up-to-n-of (abs right-charge-sum) electrons with [pxcor = max-pxcor]
      if count surplus-e < abs right-charge-sum [
        init-holes-at-x-cor (right-charge-sum - count surplus-e) max-pxcor
      ]
    ...
    right-charge-sum > 0  [
      init-electrons-at-x-cor right-charge-sum max-pxcor
    ]
  )
...
to init-electrons-at-x-cor [N  x-cor]
  create-electrons N [
    electron-init
    set xcor x-cor + random-float 1 - 0.5
    set ycor random-ycor]
end


to init-holes-at-x-cor [N x-cor]
  create-holes N [
    hole-init
    set xcor x-cor + random-float 1 - 0.5
    set ycor random-ycor]
...
to move
  ...
  if random-float 1 < scatter-prob * time-step [scatter]
...
to update-force-and-velocity
  let new-fx (charge * force-from-voltage)
