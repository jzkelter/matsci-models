...
;  ]
;
;  report u + (charge * external-voltage-at xcor)
...
;;  let left-meter-potential [e-potential] of item ((floor xcor) - min-pxcor) sorted-meters
;;  let right-meter-potential [e-potential] of item ((ceiling xcor) - min-pxcor) sorted-met
;  (ifelse
;    left-meter = nobody [report charge * ([e-potential] of right-meter +  external-voltage-at xcor)]
;    right-meter = nobody [report charge * ([e-potential] of left-meter + external-voltage-at xcor)]
;    right-meter = left-meter [report charge * ([e-potential] of left-meter + external-voltage-at xcor)]
;    [ ;; else commands
...
;      report charge * (((1 - del-x) *  [e-potential] of left-meter) + (del-x * [e-potential] of right-meter) +  external-voltage-at xcor)
;    ]
;    )
;;  report left-right-distance
;end
...
;  let right-meter-potential [e-potential] of item ((ceiling xcor) - min-pxcor) sorted-met
  (ifelse
    ...
    left-meter = nobody [report charge * ([running-ave-potential] of right-meter +  external-voltage-at xcor)]
    right-meter = nobody [report charge * ([running-ave-potential] of left-meter + external-voltage-at xcor)]
    right-meter = left-meter [report charge * ([running-ave-potential] of left-meter + external-voltage-at xcor)]

    [ ;; else commands
      let del-x (xcor - [xcor] of left-meter) / ([xcor] of right-meter - [xcor] of left-meter) ;  fraction of the way between left and right meters
      report charge * (((1 - del-x) *  [running-ave-potential] of left-meter) + (del-x * [running-ave-potential] of right-meter) +  external-voltage-at xcor)
    ]
    )
;  report left-right-distance
end

to-report external-voltage-at [x-cor]
  report (- bias) * (x-cor - min-pxcor) / world-width   ;; forward bias is actually a negative voltage
end
...
PENS
"Instanteous Potential" 1.0 0 -5298144 true "" ";foreach sort-on [xcor] meters \n;[ m -> ask m [plotxy xcor e-potential]]\n"
...
"net potential" 1.0 0 -2674135 true "" "foreach sort-on [xcor] meters \n[ m -> ask m [plotxy xcor (running-ave-potential + external-voltage-at xcor)]]\n"
...
"x-axis" 1.0 0 -4539718 true "" "plot-x-axis"
"y-axis" 1.0 0 -4539718 true "" "plot-y-axis"
"instantaneous" 1.0 0 -5298144 true "" "foreach sort-on [xcor] meters \n[ m -> ask m [plotxy xcor e-field]]"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "if ticks > 100 [plotxy ticks (charge-flow / (ticks - 99))]"
...
<experiments>
  <experiment name="experiment" repetitions="10" sequentialRunOrder="false" runMetricsEveryStep="false">
    ...
    <steppedValueSet variable="bias" first="7" step="1" last="15"/>
