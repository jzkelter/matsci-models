...
to go
  (ifelse
    mouse-action = "drawing-function" [
      draw-function
      display]
    mouse-action = "moving-particle" [
      drag-particle
      move-particle
      tick-advance time-step
      update-plots]
    mouse-action = "drag-particle-only" [
      drag-particle
      display])
...
to draw-function
  if mouse-down? [
    ask min-one-of points [abs(xcor - mouse-xcor)] [set ycor mouse-ycor]]
end

to move-particle
  ask particle2 [

;    set vx sign vx * sqrt ((vx ^ 2) * (1 - ke-dissapation * time-step))
;    set vx vx + (force-at-x xcor) * time-step
    set vx vx + (force-at-x xcor - dissapation-force * sign vx) * time-step
    fd vx
  ]
end
...
to-report force-at-x [x]
  ifelse x = round x  ;; if the particle is exactly on one of the points of the function spline
...
mouse-action
mouse-action
"drawing-function" "moving-particle" "drag-particle-only"
1
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot [xcor] of particle2"
