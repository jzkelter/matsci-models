...
to setup-atoms
  if initial-config = "HCP" [
    ...
    let min-energy-dist (2 ^ (1 / 6)) * diameter ;this is the distance with minimum energy
    let row-dist min-energy-dist
    let col-dist min-energy-dist * sqrt 3 / 2
    let ypos (- l * col-dist / 2) ;the y position of the first atom
