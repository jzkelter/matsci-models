...
to setup
  ...
  set diameter 2.3
  set max-move-dist diameter * .1
  set cutoff-dist 10 * diameter
...
to go
  ...
  ;tune the max-move-dist to adjust the acceptance rate every NUM-ATOMS move attempts
...
to-report calc-v
  ...
  ask other turtles [ ; in-radius cutoff-dist [
    ...
    let vi calc-v-with myself
    set v v + vi
    set q q + charge
  ]
 ; print q

  let coulomb-correction charge * q / cutoff-dist

  report v ;- coulomb-correction
end

to-report calc-v-with [other-atom]
  let A 100
  let alpha 2
  let B 10
  let r distance other-atom
  let q1 charge
  let q2 [charge] of other-atom
  report 100 * (A * exp(-1 * alpha * r) + B * q1 * q2 / r)
end
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot max-move-dist"
...
2. Try adding a second type of atom that has a different potential well depth (a different ϵ) when interacting with other atoms of its same type and with atoms of the different type. (ϵ11 could be for type 1 atoms interacting with themselves, ϵ22 could be for type 2 atoms interacting with themselves, and ϵ12 could be for atoms of type 1 and 2 interacting with each other).
