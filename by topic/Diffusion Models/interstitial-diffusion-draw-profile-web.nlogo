...
- When a diffusing atom enters an interstitial site, try having them lattice atoms move slightly away from the center of that site. Then, when a diffusing atom jumps, make the probability of jumping depend on how much space there is to jump through, or how much space there is at each interstitial site. 

## NETLOGO FEATURES

This model uses the mouse-xcor and mouse-ycor primitives which report the position of the mouse in order to draw the concentration profile. To get good statistics on the concentration profile, this model also uses many patches in the y-direction which extend down the screen past what is visible without scrolling. This is unusual in NetLogo models, but there is nothing wrong with it.
